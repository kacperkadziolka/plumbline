...
async def test_export_csv_trade_row_values(session: AsyncSession) -> None:
    policy_repo = PolicyRepository(session)
    policy = await policy_repo.save("test", VALID_YAML, "hash3")
    await session.flush()
    ...
    data_lines = [line for line in csv_text.strip().split("\n") if not line.startswith("#")]
    # Skip header row, parse trade rows
    trade_rows = data_lines[1:]
    assert len(trade_rows) == 2

    # Parse CSV values and verify
    eimi_row = next(row for row in trade_rows if row.startswith("EIMI.AS"))
    parts = eimi_row.split(",")
    assert parts[0] == "EIMI.AS"
    assert Decimal(parts[1]) == Decimal("400.00")
    assert Decimal(parts[2]) == Decimal("0.10")  # current_weight
    assert Decimal(parts[3]) == Decimal("0.40")  # target_weight
    assert Decimal(parts[4]) == Decimal("0.30")  # gap


async def test_export_csv_with_empty_trades(session: AsyncSession) -> None:
    policy_repo = PolicyRepository(session)
    policy = await policy_repo.save("test", VALID_YAML, "hash4")
    await session.flush()
    ...
    data_lines = [line for line in csv_text.strip().split("\n") if not line.startswith("#")]
    assert data_lines[0] == "ticker,buy_amount,current_weight,target_weight,gap"
    assert len(data_lines) == 1  # header only, no trade rows
