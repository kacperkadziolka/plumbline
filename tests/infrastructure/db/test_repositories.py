...
class TestGetOrCreateAsset:
    async def test_creates_new_asset(self, session: AsyncSession) -> None:
    ...
    async def test_returns_existing_asset(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
...
class TestGetAssetByTicker:
    async def test_returns_asset_when_found(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
...
class TestCreateSnapshot:
    async def test_creates_snapshot_with_new_assets(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
    ...
    async def test_creates_snapshot_with_existing_assets(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
    ...
    async def test_creates_snapshot_with_duplicate_tickers(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
        as_of_date = datetime(2024, 1, 15)
    ...
    async def test_creates_empty_snapshot(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
        as_of_date = datetime(2024, 1, 15)
...
class TestGetSnapshot:
    async def test_returns_snapshot_with_positions_and_assets(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
        as_of_date = datetime(2024, 1, 15)
    ...
    async def test_raises_when_not_found(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
        ...
        assert "99999" in exc_info.value.details


class TestGetLatestSnapshot:
    async def test_returns_most_recent_by_as_of_date(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
    ...
    async def test_returns_none_when_empty(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
...
class TestListSnapshots:
    async def test_returns_ordered_by_date_descending(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
    ...
    async def test_returns_empty_list_when_no_snapshots(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
...
class TestDeleteSnapshot:
    async def test_deletes_snapshot_and_positions(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
    ...
    async def test_does_not_delete_assets(self, session: AsyncSession) -> None:
        repo = HoldingsRepository(session)
